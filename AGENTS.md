...
- `scripts/hermes-agent-providers.mjs` — development-executor provider contract shared by the console/readiness tools: `EXECUTOR_TYPES` allowlist (`codex`, `devin`), truthful readiness vocabulary (`ready`/`configured`/`unavailable`/`unknown`, never "connected"), codex CLI probe, devin credential-presence check, and the bounded Devin API adapter shape (bounded prompt, idempotency key, bounded session parse). The worker implements the same contract inline.
