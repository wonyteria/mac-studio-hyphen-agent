...
command:
  ...
  install       macOS LaunchAgent plist 구성 + 로드 (명시 실행 시에만 시스템 변경)
  uninstall     LaunchAgent unload + plist 제거
  status        마지막 동기화 상태 + LaunchAgent 등록 상태 확인 (읽기 전용)
...
function parseArgs(argv) {
  const options = { command: "sync", dryRun: false, json: false, noLoad: false };
...
// current Node executable; every other path is the explicit --source /
// --destination / --status value baked in verbatim.
function launchdPlist({ source, destination, statusPath }) {
