...
- [x] Which connector should bring the Studio business registry into the agent workspace for true cross-project briefings / owner: operator / resolved: the `studio_priorities`/`studio_blockers` types render deterministic priorities/blocker views in-server from the `HERMES_BUSINESS_REGISTRY` mount — see `BRIEFING.md`.
