...
- New/changed components: Hyphen Studio Agent branding, empty-state preset launcher (오늘 브리핑 / 오늘 우선순위 / 막힌 프로젝트 / Mac 상태 점검 / 배포 상태 확인), truthful connection indicator, project context on the active request, per-type approval consequence copy, `빠른 판단` observation panel, small shadow-evidence summary, business-data freshness pill.
