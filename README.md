...
- empty-state action presets (오늘 우선순위 / 막힌 프로젝트 / Mac 상태 점검 / 배포 상태 확인) that only prefill the composer — never auto-submit or bypass approval; the priority/blocked presets are true cross-project actions scoped to 전체 Hyphen Studio via the `studio_priorities`/`studio_blockers` types, while Mac/deployment presets stay selected-project scoped and the composer warns with type-specific Korean guidance when the selected project lacks the required capability (the backend gate still decides)
