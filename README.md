...
- password-protected internal chat UI branded as the Hyphen Studio Agent workspace, redesigned for narrow/mobile viewports — warm-neutral surfaces, no horizontal page scroll at ≥320px, 44px touch targets, visible focus rings, reduced-motion safe
- responsive shell: desktop sidebar plus a mobile request-history drawer (menu button → 홈 · 빠른 작업 / 새 요청 / 지난 요청 / 새로고침 / 로그아웃, closes on selection, scrim tap, or Escape) and a compact header whose connection/business-freshness/observation pills wrap instead of overflowing
- persistent Home overview (홈 · 빠른 작업) reachable from desktop and mobile: Korean greeting, live state chips (connection, 사업 데이터 freshness, 빠른 판단 coverage), and grouped quick actions — 사업 (전체 Hyphen Studio 기준), 운영, 개발 (선택한 프로젝트 기준)
- home quick-action presets (오늘 브리핑 / 오늘 우선순위 / 막힌 프로젝트 / 사업 현황 갱신 점검 / Mac 상태 점검 / 배포 상태 확인 / 프로젝트 점검 / 개발 요청) that only prefill the composer — never auto-submit or bypass approval; the briefing/priority/blocked/audit presets are true cross-project actions scoped to 전체 Hyphen Studio via the `studio_overview`/`studio_priorities`/`studio_blockers`/`studio_evidence_audit` types, while Mac/deployment/inspect/development presets stay selected-project scoped and the composer warns with type-specific Korean guidance when the selected project lacks the required capability (the backend gate still decides)
- readable results: text results render in a wrapping sans-serif panel (no monospace dumps); `studio_evidence_audit` renders a structured card — summary metrics, priority counts, and per-project collapsible items with Korean missing-field chips and next actions — backed by a bounded `audit-v1` view model stored in `request.briefing.audit`; older records without it fall back to the text panel
- collapsible `빠른 판단` observation and `최근 기록` event-history regions keep result blocks scannable
...
- truthful connection indicator driven by real API results, plus visible project context on the active request and a plain-Korean scope line in the composer (범위: 전체 Hyphen Studio vs 프로젝트: <name>)
