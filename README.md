...
- responsive shell: desktop sidebar plus a mobile request-history drawer (menu button → 홈 · 빠른 작업 / 새 요청 / 지난 요청 / 새로고침 / 로그아웃, closes on selection, scrim tap, or Escape) and a compact header whose connection/business-freshness/observation pills wrap instead of overflowing — the connection pill is driven by real API results, never a static indicator
